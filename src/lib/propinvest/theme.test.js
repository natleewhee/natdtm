// src/lib/propinvest/theme.test.js
...
test('parseMoney: a leading "-" is stripped, NOT preserved (documented limitation)', () => {
  // parseMoney is built for money fields, which are never negative, so it
  // strips a leading minus sign rather than rejecting it — this means it
  // must never be used directly for a field where a negative value is a
  // real, catchable input error (e.g. downpayment %). page.js uses a
  // separate sign-preserving numSigned() helper for exactly that reason.
