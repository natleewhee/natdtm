...
const BUNDLE_KEYS = ['conservative', 'base', 'optimistic']

// Resolve every move to a dated delta in projectSegmented's input shape,
// threading same-year sale proceeds into a same-year buy.
function resolveTimeline(moves = []) {
  const sorted = [...moves].sort((a, b) => (Number(a.year) || 0) - (Number(b.year) || 0))
  ...
  for (const move of sorted) {
    ...
    dated.push({
      ...
      payoff: d.payoff
        ? { year: d.payoff.year, monthlyContribution: d.payoff.monthlyContributionDelta || 0 }
        : undefined,
    })
    for (const extra of (d.datedExtras || [])) {
      dated.push({
        year: Math.max(0, Math.trunc(Number(extra.year) || 0)),
...
function labelRead(bandBase, bandConservative, reference) {
...
export function runScenario(baseState = {}, scenario = {}, bundles = {}, retireAssumptions = {}, reference = 0) {
  ...
  const { dated, warnings, property: propChange, mortgage: mortChange } = resolveTimeline(scenario.moves || [])
